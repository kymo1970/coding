...
def planetGravity(body, gravity, damping, dt):
    ...
    sq_dist = body.position.get_dist_sqrd((400, 400))
    g = (
        (body.position - pymunk.Vec2d(400, 400))
        * -gravityStrength
...
def add_box(space):
    ...
    # starting position.
    r = body.position.get_distance((400, 400))
    v = math.sqrt(gravityStrength / r) / r
    body.velocity = (body.position - pymunk.Vec2d(400, 400)).perpendicular() * v
...
while True:
    ...
    pygame.draw.circle(screen, pygame.Color("yellow"), (400, 400), 10)
